...
import org.hyl.bucket.web.commons.rest.RestUtil;
...
public interface ThrowableAdviceTrait {
    ...
    default Message handleBindException(final BindException e) {
        return RestUtil.error(e.getClass().getName(), HttpStatus.BAD_REQUEST.value(), e.getBindingResult().getFieldError().getDefaultMessage());
    }
    ...
    default Message handleInternalServerErrorException(final InternalServerErrorException e) {
        return RestUtil.error(e.getType(), e.getClass().getName(), e.getState(), e.getMessage(), e.getData(), e.getParams());
    }
    ...
    default Message handleThrowable(final Throwable throwable) {
        return RestUtil.error(throwable.getClass().getName());
    }
}
